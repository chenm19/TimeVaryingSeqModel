...
class MinimalRNNCell(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.Q = self.add_weight(
            shape=(self.units, self.q * self.units +self.units),  # theta-matrix to be optimized
            initializer=self.kernel_initializer,
            name='theta')
        if self.use_bias:
...
model3.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=lr, decay=0.00),
                   loss='categorical_crossentropy', metrics=['accuracy'])

y_train =  to_categorical(y_full_train, num_classes=num_classes)
history3=model3.fit(x_full_train, y_train, epochs=epoch, batch_size=batch_size, verbose=0)